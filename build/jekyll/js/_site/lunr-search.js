...
function getMatchesInContext(result, searchPhrase, item){
  ...
  for (var oneMatchedSearchTerm in matchPositionInfo){
    for (var oneMatchType in matchPositionInfo[oneMatchedSearchTerm]){
      if (oneMatchType === 'keywords') {
        //keyword matches
        const keywordArray = item.keywords.split(', ');
        const keywordMatchString = '.*' + oneMatchedSearchTerm + '.*';
  ...
  for (var oneMatchedSearchTerm in matchPositionInfo){
    for (var oneMatchType in matchPositionInfo[oneMatchedSearchTerm]){
      if (oneMatchType != 'keywords') {
        for (var oneMatchPosition in matchPositionInfo[oneMatchedSearchTerm][oneMatchType].position){
          var onePosition = matchPositionInfo[oneMatchedSearchTerm][oneMatchType].position[oneMatchPosition];
          if (numberOfPrintedResults < resultsPerPageLimit){
            numberOfPrintedResults++;
            returnString += '<li>' + highlightMatches(item.content.substring(onePosition[0]-70, onePosition[0]+onePosition[1]+70), oneMatchedSearchTerm)  + '</li>';
          }
        }
      }
    }
...
function highlightMatches(content, searchTerm){
  ...
  var matches = content.match(matchRegex);
  console.log(matches);
